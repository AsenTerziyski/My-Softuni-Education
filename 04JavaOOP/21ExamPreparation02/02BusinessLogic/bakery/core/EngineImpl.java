...
public class EngineImpl implements Engine {
    ...
    private String processInput() throws IOException {
        ...
        switch (command) {
            case AddDrink:
                result = this.controller.addDrink(data[0], data[1], Integer.parseInt(data[2]), data[3]);
                break;
            case AddFood:
                result = this.controller.addFood(data[0], data[1], Double.parseDouble(data[2]));
                break;
            case AddTable:
                result = this.controller.addTable(data[0], Integer.parseInt(data[1]), Integer.parseInt(data[2]));
                break;
            case ReserveTable:
